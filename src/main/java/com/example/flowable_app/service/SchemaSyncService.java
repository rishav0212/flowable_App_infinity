...
public class SchemaSyncService {
    ...
    public void syncFormDefinition(String jsonBody) {
        try {
            ...
            // POSTGRES UPDATE: Use 'current_schema' logic
            List<String> existingColumns = dsl.select(DSL.field("column_name"))
                    .from("information_schema.columns")
                    .where(DSL.field("table_name").eq(tableName))
                    .and(DSL.field("table_schema").eq(DSL.function("current_schema", String.class)))
    ...
    private org.jooq.DataType<?> getSqlType(String type) {
        switch (type) {
            ...
            // POSTGRES UPDATE: Use JSONB for better performance
            case "file":
            case "tags":
            case "select":
            ...
            case "textarea":
                // In Postgres, TEXT is preferred over CLOB
